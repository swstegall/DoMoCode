...
struct WedgeSurfaceTests {
    ...
    func createFailureIsReported() async throws {
        ...
        let client = await WedgeClient.make(baseURL: stub.baseURL, token: Self.token, watching: [
    ...
    func sessionListRefreshFailureIsReported() async throws {
        ...
        let client = await WedgeClient.make(baseURL: stub.baseURL, token: Self.token)
    ...
    func diagnosticsPanelRefitsOnResize() async throws {
        ...
        let client = await WedgeClient.make(baseURL: stub.baseURL, token: Self.token, columns: 40, oracleColumns: 220)
    ...
    func forceClearRecoversAWedgedSession() async throws {
        ...
        let client = await WedgeClient.make(baseURL: "http://127.0.0.1:\(port)", token: Self.token)
        try? await Task.sleep(for: .milliseconds(800))
    ...
    func aRefusedPromptReconcilesWithTheServer() async throws {
        ...
        let client = await WedgeClient.make(baseURL: stub.baseURL, token: Self.token, watching: ["Esc aborts it"])
        try? await Task.sleep(for: .milliseconds(800))
